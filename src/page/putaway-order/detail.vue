<template>
    <div class="putaway-order pb-lg">
        <mt-header fixed :title="title">
            <!--<router-link :to="{path:'/putaway-order',query:{warehouse_id:this.$route.query.warehouse_id}}" slot="left">
            ...
            <!--<button slot="right" v-if="putawayOrderDataShow&&active==='tab-container1'" @click="complete">完成上架</button>-->
            <!--<button slot="right" v-if="butShow&&!putawayOrderDataShow" @click="all_submit">生成上架单</button>-->
            <request-button slot="right" v-if="putawayOrderDataShow&&active==='tab-container1'" :request="complete">完成上架</request-button>
            <request-button slot="right" v-if="butShow&&!putawayOrderDataShow" :request="all_submit">生成上架单</request-button>
        </mt-header>
                ...
                <mt-tab-container-item id="tab-container1" class="pt-lg">
                    <div>
                        <mt-cell style="position: fixed;z-index: 2;width: 100%;max-width: 640px;">
                            <table><tr><td width="20%">排序</td><td width="30%">货位</td><td width="20%">SKU</td><td width="20%">数量</td></tr></table>
                        </mt-cell>
                                ...
                                <mt-cell :class="[line_now === index?'height-line':'',(index%2===0&&index!==line_now)?'line-bg':'']"
                                    ...
                                    <table>
                                        <tr :class="[`${row.actual_quantity}`===`${row.quantity}`?'green':'',(`${row.actual_quantity}`!==`${row.quantity}`)&&(index < line_now)?'red':'']">
                                            <td width="20%">{{row.sequence_number}}</td>
                                            <td width="30%">{{row.warehouse_cargo_id}}</td>
                                            <td width="30%">{{row.sku}}</td>
                                            <td width="20%">{{row.actual_quantity}}/{{row.quantity}}</td></tr>
                                    </table>
                                </mt-cell>
                            </div>
                        </virtual-list>
        ...
        <div v-else>
            <div class="content">
                ...
                <tab-table titleName="SKU列表" :code-count="combineCodeDataLength">
                    <tr><td>SKU</td><td>货位</td><td>编号</td><td>数量</td></tr>
                    <tr v-for="(row,item) in combineCodeData" :key="item">
                        <td>{{row.sku}}</td>
                        <td>{{row.warehouse_cargo_id}}</td>
            ...
            <msgbox v-if="showBox" :titleName="`${this.combine_code}扫描结果`" @close-box="close_box" @submit="submit_msgbox">
                <table>
                    <tr>
                        <td @click="select_all"><input type="checkbox" v-model="select_show"></td>
                        <td>批次</td><td>采购单ID</td><td>数量</td></tr>
                    <tr v-for="(row,item) in skuData" :key="item">
...
//                    Toast({message:`请至少选择一个`,position: 'top', className: 'icon-index',duration: 1500});
            ...
            all_submit(){
                ...
                return this.$http(putaway_order_create,{data:data}).then(res=>{
                    ...
                    console.log('生成数据',this.putawayOrderData);
            ...
            make_off(){
                ...
                let data ={
                    ...
                    putaway_waiting_goods_id:this.putaway_waiting_goods_id,
