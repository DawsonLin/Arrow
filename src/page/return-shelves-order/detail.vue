<template>
    <div class="putaway-order pb-lg">
        <mt-header fixed :title="title">
            <!--<router-link :to="{path:'/return-shelves-order',query:{warehouse_id:this.$route.query.warehouse_id}}" slot="left">-->
            ...
            <div class="partition" slot="right" @click="changeState">
                上架路径
            </div>
        </mt-header>
        ...
        <msgbox-sku ref="sku_goods" v-model="skuShow" :sku="sku_name" :warehouse_id="this.$route.query.warehouse_id"
                    @get-img="getimg"
                    @close-box="close_sku_box">
        </msgbox-sku>
    </div>
</template>
...
<script>
    ...
    import{api_return_shelves_read,api_return_shelves_put,putaway_order_read,putaway_order_save,putaway_order_status,api_putaway_look_goods} from '../../api/putaway-order';
    /*
     * 后端:刘志勇
     * */
    export default{
        ...
        methods:{
            backpath(){
                if(this.active==='tab-container2'){
                    ...
                    this.$router.push({path:'/return-shelves-order',query:{warehouse_id:this.$route.query.warehouse_id}})
            ...
            line_off(item,num){
                ...
                this.putawayOrderData.forEach((row,index)=>{
                    if(row.id===item.id){
                        ...
                        this.listData.sku = row.sku;
                        this.listData.sku_alia = row.sku_alias;
            ...
            get_putaway_order(id){
                this.$http(api_return_shelves_read,id).then(res=>{
            ...
            make_off(){
                ...
                let data ={
                    ...
                    return_shelves_id :this.orderNumber,
                    warehouse_cargo_code :this.warehouse_cargo,
                    sku :this.sku,
                    quantity :this.actual_quantity,
                };
                return this.$http(api_return_shelves_put,this.orderNumber,data).then(res=>{
                    ...
                    if(!find){
                        ...
                        for(let i = this.line_now; i < this.putawayOrderData.length; i++){
                            ...
                            if(quantity === 0){
                                ...
                                this.listData = {
                                    ...
                                    sku:this.putawayOrderData[i].sku,
                                    sku_alia:this.putawayOrderData[i].sku_alias,
                        ...
                        if(this.line_now===this.putawayOrderData.length){
                            ...
                            this.listData = {
                                ...
                                sku:this.putawayOrderData[index].sku,
                                sku_alia:this.putawayOrderData[index].sku_alias,
                ...
                if(val==='tab2'){
                    this.putawayOrderData.forEach((row,index)=>{
                        if(index===this.line_now) {
                            this.listData.warehouse_cargo_code = row.warehouse_cargo_code;
                            this.listData.sku = row.sku;
                            this.listData.sku_alias = row.sku_alias;
                            this.listData.quantity = row.quantity_left;
                            this.listData.sort = row.sort;
                            this.actual_quantity = row.quantity_left;
                            this.detail_id = row.id;
